...
pub struct TuiFrontend {
    ...
    /// (label, is_active, status_symbol, unread, sound_enabled, tts_enabled)
    pub session_labels: Vec<(String, bool, String, usize, bool, bool)>,
    /// Whether to use compact tab display
    pub compact_tabs: bool,
    /// Show first-run setup screen (no connection.toml found)
    ...
    pub login_wizard: Option<login_wizard::ProfilePicker>,
}
...
impl TuiFrontend {
    ...
    /// Swap out the current WidgetManager and return it, replacing with the given one.
    /// Used for per-session widget state isolation on session switch.
    pub fn swap_widget_manager(&mut self, incoming: WidgetManager) -> WidgetManager {
        std::mem::replace(&mut self.widget_manager, incoming)
    }
}
